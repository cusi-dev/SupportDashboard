class Dashing.CusiProgressBars extends Dashing.Widget
  ...
  drawWidget: (progress_items) ->
    ...
    if progress_items.length == 0
      ...
      progress_items.forEach (item) =>
        ...
        if elem.length
          @animateProgressBarContent(elem[0], parseFloat(elem[0].text), #parseFloat(elem[0].style.width),
                                    parseFloat(item.progress), item.value, 100) #1000)
        ++counter
